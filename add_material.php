...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['courseFile'])) {
    $uploadDir = 'Uploads_material/' . $course_name;
    ...
    $allowedTypes = ['pdf', 'doc', 'docx', 'ppt', 'pptx', 'jpg', 'jpeg', 'png', 'txt', 'xls', 'xlsx', 'csv'];
    $fileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

    if (in_array($fileType, $allowedTypes)) {
        if (move_uploaded_file($_FILES['courseFile']['tmp_name'], $targetFile)) {
            echo "<script>alert('File uploaded successfully!'); window.location.href='courses_material.php?success=1';</script>";
        } else {
            echo "<script>alert('Error uploading file.');</script>";
        }
    } else {
        echo "<script>alert('File type not allowed. Please upload a valid study material file.');</script>";
    }
}
?>
...
<body>
    <div class="upload-container">
        <h1>Upload File for <?php echo htmlspecialchars($course_name); ?></h1>
